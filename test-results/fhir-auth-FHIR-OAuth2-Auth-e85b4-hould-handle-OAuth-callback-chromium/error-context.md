...
- Name: fhir-auth.spec.js >> FHIR OAuth2 Authentication >> GET /api/fhir/auth/callback should handle OAuth callback
- Location: tests/e2e/fhir-auth.spec.js:63:3
...
Received value: {"error": "Authentication required"}
```
...
```ts
  1   | /**
  2   |  * FHIR Authentication E2E Tests
  3   |  * Tests OAuth2 flows, token refresh, and portal credential management
  4   |  */
  5   | 
  6   | import { test, expect } from '@playwright/test';
  7   | 
  8   | test.describe('FHIR OAuth2 Authentication', () => {
  9   |   let authToken;
  10  |   let credentialId;
  11  | 
  12  |   test.beforeEach(async ({ request }) => {
  13  |     // Setup: login first (auth required for FHIR routes)
