...
# Build & Notarize Script for MyTreatmentPath (with certificate fix)
# Forces use of login keychain to avoid ambiguity
...
echo "   Version: 0.1.0"
echo "   Identity: DA7B788C70B5FD6A1507950B3F31B7ACC32EDA1B"
echo "   Keychain: login.keychain-db (to avoid ambiguity)"
...
# Set environment variables to force certificate selection
export CSC_NAME="DA7B788C70B5FD6A1507950B3F31B7ACC32EDA1B"
...
if [ -f "build/mac-arm64/MyTreatmentPath.app/Contents/MacOS/MyTreatmentPath" ]; then
    echo ""
    echo "✅ Build complete!"
    echo ""
    ...
    if [ -n "$APPLE_ID" ]; then
        ...
        if spctl -a -vvv -t install build/mac-arm64/MyTreatmentPath.app 2>&1 | grep -q "Notarized Developer ID"; then
