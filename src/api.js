...
const isSubscriptionsAvailable = typeof window !== 'undefined' &&
  window.electron && window.electron.subscriptions;

export async function getSubscriptionCategories() {
  if (isSubscriptionsAvailable) return window.electron.subscriptions.getCategories();
  const res = await fetch('/api/subscriptions/categories', { credentials: 'include' });
  return res.json();
}

export async function getSubscriptions(filters = {}) {
  if (isSubscriptionsAvailable) return window.electron.subscriptions.list(filters);
  const params = new URLSearchParams(filters).toString();
  const res = await fetch(`/api/subscriptions${params ? '?' + params : ''}`, { credentials: 'include' });
  return res.json();
}

export async function getSubscriptionSummary() {
  if (isSubscriptionsAvailable) return window.electron.subscriptions.summary();
  const res = await fetch('/api/subscriptions/summary', { credentials: 'include' });
  return res.json();
}

export async function addSubscription(data) {
  if (isSubscriptionsAvailable) return window.electron.subscriptions.add(data);
...
export async function updateSubscription(id, data) {
  if (isSubscriptionsAvailable) return window.electron.subscriptions.update(id, data);
...
export async function deleteSubscription(id) {
  if (isSubscriptionsAvailable) return window.electron.subscriptions.remove(id);
  const res = await fetch(`/api/subscriptions/${id}`, {
    method: 'DELETE',
    credentials: 'include',
  });
...
export async function getSubscriptionPayments(subscriptionId) {
  if (isSubscriptionsAvailable) return window.electron.subscriptions.listPayments(subscriptionId);
  const res = await fetch(`/api/subscriptions/${subscriptionId}/payments`, { credentials: 'include' });
  return res.json();
}

export async function addSubscriptionPayment(subscriptionId, data) {
  if (isSubscriptionsAvailable) return window.electron.subscriptions.addPayment(subscriptionId, data);
